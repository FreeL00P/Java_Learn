...
public class MyPanel extends JPanel implements KeyListener,Runnable {
    ...
    int enemyTankSize=10;//敌人坦克数量
    //定义三张图片 用于显示爆炸效果
    Image image1=null;
    Image image2=null;
    Image image3=null;
    public MyPanel(String key){
        nodes= Record.getInfo();//获取上局信息 getInfo()返回一个nodes
        hero=new Hero(50,100);//初始化主角坦克
    ...
    public void showInfo(Graphics g){
        ...
        drawTank(1020 , 60, g, 0, 1);//敌人坦克
        g.setColor(Color.black);
        g.drawString(String.valueOf(Record.getWreckEnemyTankNum()),1080,100);
        ...
        switch (enemyTank.getDirect()){
            ...
            case 1:
                ...
                if(shot.getX()>enemyTank.getX()&&shot.getX()<enemyTank.getX()+40 &&shot.getY()>enemyTank.getY()&&shot.getY()<enemyTank.getY()+60
                    ...
                    bombs.add(bomb);
                    //因为enemyTank 可以是hero 也可以是EnemyTank 所以需要判断运行类型
                    //否则会出现自杀加一分的情况
                    if (enemyTank instanceof EnemyTank){
                        Record.addWreckEnemyTankNum();//击毁敌人坦克的数量加1
            ...
            case 3:
                ...
                if(shot.getX()>enemyTank.getX()&&shot.getX()<enemyTank.getX()+60 &&shot.getY()>enemyTank.getY()&&shot.getY()<enemyTank.getY()+40
                    ...
                    if (enemyTank instanceof EnemyTank){
                        Record.addWreckEnemyTankNum();//击毁敌人坦克的数量加1
                        enemyTanks.remove(enemyTank);//集合中删除敌人对象
                    }
                }
                break;
        }
